...
from .evaluation import build_error_report, compute_metrics, save_json, save_predictions
...
from .model import LinearMultiLabelModel, train_linear_multilabel
...
def train_gold_entity_baseline(config: ExperimentConfig) -> Dict[str, object]:
    ...
    model = train_linear_multilabel(
        ...
        [example["labels"] for example in train_examples],
        config,
    )
...
def predict_with_gold_entities(
    ...
    for document in documents:
        ...
        for label_row, pair in zip(predicted_labels, pairs):
            subject_name, _, object_name, _ = pair
